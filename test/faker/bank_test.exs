defmodule Faker.BankTest do
  use ExUnit.Case, async: true

  test "iban/1 returns iban with valid length" do
    assert String.length(Faker.Bank.iban("NL")) == 18
  end

  test "iban/1 with country code returns iban for that country" do
    assert "BE" <> _tail = Faker.Bank.iban("BE")
  end

  test "iban/0 returns random iban" do
    assert Regex.match? ~r/^[A-Z]{2}\d{2}[A-Z0-9]+$/, Faker.Bank.iban
  end
  ...
  test "iban/2 generates iban starting with given components" do
    assert Regex.match?(~r/NL\d{2}INGB\d{10}/, Faker.Bank.iban("NL", ["INGB"]))
  end
end
